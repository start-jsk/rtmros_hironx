...
NAME_LOGFILE=${TMPDIR}/robot-compile-setup-`date +"%Y%m%d-%H%M%S"`.log
...
# Move files in the tarball above to ${HRPSYS_VER}
(cd ${TMPDIR}; tar -xvzf opt-jsk-base.tgz; mv ./opt/jsk/include ${TMPDIR}/opt/jsk/${HRPSYS_VER}; mv ./opt/jsk/lib ${TMPDIR}/opt/jsk/${HRPSYS_VER})
...
# If Hiro, fetch tarball of /opt/hiro to the ${TMPDIR}/hiro_wrl.tgz on the local machine, then extract the content.
IS_QNX_HIRO=is_qnx_hiro
commands_check_hiro="
  ls /opt/hiro && (echo 'Ok, the robot on QNX is Hiro.' && tar cfvz $TMPDIR/hiro_wrl.tgz /opt/jsk/etc/HIRONX && true) || (echo 'No, this robot on QNX is NXO.'; false)" && (echo 'true' > $TMPDIR/$IS_QNX_HIRO);
  "
...
if [ -e $TMPDIR/$IS_QNX_HIRO ];
then
  (scp $USERNAME_QNX@$HOSTNAME_QNX:${TMPDIR}/hiro_wrl.tgz ${TMPDIR}/hiro_wrl.tgz; echo 'Create a tarball for hiro model files.' && cd ${TMPDIR} && tar xfvz ${TMPDIR}/hiro_wrl.tgz ${TMPDIR}/opt/jsk/${HRPSYS_VER}/etc/) && (echo 'Model files of Hiro are retrieved from robot.' && ls ${TMPDIR} || echo "The robot you're dealing with now is NEXTAGE."
else
...
scp $USERNAME_QNX@$HOSTNAME_QNX:/opt/jsk/lib/RobotHardware.so ${TMPDIR}
HRPSYS_PREV_INSTALLED=$(strings ${TMPDIR}/RobotHardware.so | grep ^[0-9]*\\.[0-9]*\\.[0-9]*$)
...
commands="
  ...
  test -L /opt/jsk && su -c 'rm -fr /opt/jsk' || (mkdir ${TMPDIR}/$HRPSYS_PREV_INSTALLED && su -c 'mv /opt/jsk/* ${TMPDIR}/$HRPSYS_PREV_INSTALLED'; echo 'ls /opt/jsk' && ls /opt/jsk && su -c 'mv ${TMPDIR}/$HRPSYS_PREV_INSTALLED /opt/jsk');
  echo \"* setup /opt/jsk/$HRPSYS_VER directory *\";
  cd /;
  su -c 'tar -xvzf ${TMPDIR}/opt-jsk-base-model.tgz';
...
if [ "$REPLY" == "y" ]; then
    echo "scp ${TMPDIR}/opt-jsk-base-model.tgz into $HOSTNAME_QNX"
    scp ${TMPDIR}/opt-jsk-base-model.tgz $USERNAME_QNX@$HOSTNAME_QNX:${TMPDIR}
