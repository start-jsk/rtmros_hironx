...
wget 'https://docs.google.com/uc?authuser=0&id=0B5hXrFUpyR2iZS0tQlFyXzhjaGc&export=download' -O ${TMP_FOLDER}/opt-jsk-base.tgz || echo "ERROR:: Failed to download opt-jsk-base.tgz."
# Move files in the tarball above to ${HRPSYS_VER}
(cd ${TMP_FOLDER}; tar -xvzf opt-jsk-base.tgz; mv ./opt/jsk/include ${TMP_FOLDER}/opt/jsk/${HRPSYS_VER}; mv ./opt/jsk/lib ${TMP_FOLDER}/opt/jsk/${HRPSYS_VER})
...
then
  (scp $USERNAME_QNX@$HOSTNAME_QNX:${TMP_FOLDER}/hiro_wrl.tgz ${TMP_FOLDER}/hiro_wrl.tgz; echo 'Create a tarball for hiro model files.' && cd ${TMP_FOLDER} && tar xfvz ${TMP_FOLDER}/hiro_wrl.tgz ${TMP_FOLDER}/opt/jsk/${HRPSYS_VER}/etc/) && (echo 'Model files of Hiro are retrieved from robot.' && ls ${TMP_FOLDER} || echo "The robot you're dealing with now is NEXTAGE."
else
...
tar -C ${TMP_FOLDER} -cvzf ${TMP_FOLDER}/opt-jsk-base-model.tgz ./opt/jsk/${HRPSYS_VER}/
...
commands="
  ...
  cd /;
  su -c 'tar -xvzf ${TMP_FOLDER}/opt-jsk-base-model.tgz';
