...
def hrpiob_iob():
    ...
    print re.sub('(\n)', '\n  - ', code)
    command = 'gcc -v -o %s %s -I%s -L/usr/pkg/lib -L/opt/nextage-open/lib -lhrpIo -lboost_thread -lboost_signals -lboost_filesystem -lboost_system -lboost_regex -lf2c'%(os.path.splitext(f_code_name)[0], f_code_name, d_tmp)
    print "  - ",command
